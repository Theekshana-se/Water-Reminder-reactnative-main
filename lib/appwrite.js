...
export const deleteUser = async () => {
    try {
        ...
        if (userDocuments.documents.length > 0) {
            ...
            console.log('User document deleted:', documentId);
        } else {
            console.warn('No user document found to delete.');
        }
        ...
        return { success: true, message: 'User deleted successfully (document and session).' };
    } catch (error) {
...
const getCurrentUserDocId = async () => {
  ...
  if (userDoc.documents.length === 0) throw new Error('User document not found.');
  return userDoc.documents[0].$id;
};

export const updateProfilePicture = async (profilePicUrl) => {
  const docId = await getCurrentUserDocId();
...
export const updateUserEmailAndPhone = async (email, phoneNumber = null) => {
  const docId = await getCurrentUserDocId();
  await databases.updateDocument(
    config.databaseId,
    config.userCollectionId,
...
export const updateWakeUpTime = async (time) => {
  const docId = await getCurrentUserDocId();
  await databases.updateDocument(
    config.databaseId,
    config.userCollectionId,
...
export const updateBedtime = async (time) => {
  const docId = await getCurrentUserDocId();
  await databases.updateDocument(
    config.databaseId,
    config.userCollectionId,
